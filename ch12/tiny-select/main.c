...
void sigpipeHandlerExit(int sig);
void check_client(fd_set *ready_set, int maxi);
...
void serve_dynamic(int fd, char *filename, char *cgiargs, int only_head) {
  ...
  if (Fork() == 0) {
    Signal(SIGPIPE, sigpipeHandlerExit);
    sprintf(buf, "HTTP/1.0 200 OK\r\n");
    Rio_writen(fd, buf, strlen(buf));
...
void check_client(fd_set *ready_setp, int maxi) {
  debug("check_client: entering");
  int i;
  for (i = 0; i <= maxi; ++i) {
    if (FD_ISSET(i, ready_setp)) {
      if (setjmp(env) == 1) continue;
      FD_CLR(i, ready_setp);
      doit(i);
    }
  }
